...
class SnapshotRepositoryImpl @Inject constructor(
    ...
    override fun addSnapshot(snapshotR: SnapshotRequest): Single<ResultModel> =
        firebaseDatabaseCustom.addSnapshot(snapshotR)
            .map { FBDBC ->
                FBDBC.toModel()
            }

    override fun deleteSnapshot(id:String): Observable<MutableList<SnapshotModel>> =
        firebaseDatabaseCustom.deleteSnapshot(id)
            .map { list ->
                list.toModel()
            }
}